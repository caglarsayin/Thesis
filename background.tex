...
\item[Fully Connected Star] Fully connected start are directly connected star topologies which actually known in some terminologies as mesh networks. This topology is the best topology possible which can be designed ever, because every nodes are directly connected with each other with $1$ routing distance. If there is $N$ number of node that means the bisection bandwidth is exactly $1024$, because each node has for every other nodes another links with an other bandwidth, it makes it bottleneck-less topology.
