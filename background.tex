...
\begin{description}
  ...
  \item[Degree of a Node] It is the properties of each node to imply how many nodes it is directly connected to. The note with the highest degree is applied as degree of the network.
\end{description}
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{img/multidimensional.jpg}
    \caption{A) Mesh topology example B) Torus topology example C) 3D meshes  topology example\cite{HPCfigure}}
    \label{fig:meshestorus}
\end{figure}
\begin{description}
\item[Buses and Rings]
\item[Star and Fully Connected Star]
\item[Two dimensional: Meshes, Torus] 
